...
describe('materials', () => {
    describe('prime combinations', () => {
        it('given a non-prime, errors', () => {
            expect(() => primeCombinations(as.Integer(4)))
                .toThrowError('4 is not prime.')
        })

        it('given 2, the octave, errors', () => {
            expect(() => primeCombinations(as.Integer(2)))
                .toThrowError('This process is octave-reduced so 2 does not count.')
        })

        it('given 3, errors', () => {
            expect(() => primeCombinations(as.Integer(3)))
                .toThrowError('You must provide at least 2 primes greater than the octave.')
        })

        it('given a prime limit, will return a sorted list of all combinations of two primes (excluding octaves)', () => {
            expect(primeCombinations(as.Integer(5)))
                .toEqual([
                    [ 3, 5 ],
                ].map((combination: number[]) => combination.map((as.Integer))))
